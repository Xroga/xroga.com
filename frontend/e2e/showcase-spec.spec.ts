...
test('every stored thumbnail resolves and is a real image', async ({ page, request }) => {
  for (const slug of SLUGS) {
    for (const view of ['desktop', 'tablet', 'mobile']) {
      const response = await request.get(
        `/showcase/thumbnails/${slug}-${view}.webp`
      );
  ...
  const decoded = await page.evaluate(async () => {
    ...
    image.src =
      '/showcase/thumbnails/modern-business-website-desktop.webp';
...
test('modern business: the 2026 editorial redesign and truthful demo form work', async ({ page }) => {
  ...
  await expect(
    page.getByRole('heading', {
      name: /design\. build\. ship\./i,
    })
  ).toBeVisible();

  await expect(
    page.getByText('Illustrative demo').first()
  ).toBeVisible();

  await expect(
    page.getByText(/sample testimonials for the showcase/i)
  ).toBeVisible();
  ...
  await form
    ...
    .fill('A new product marketing website.');
  ...
  await expect(
    form.getByRole('status')
  ).toContainText(/not sent or stored/i);
});
...
test('real estate: the gallery switches views and the enquiry form validates', async ({ page }) => {
  ...
  await expect(
    drawer.locator('[aria-invalid="true"]').first()
  ).toBeVisible();
...
test('mobile app: the profile screen reports real local state', async ({ page }) => {
  ...
  const savedStat = page
    .locator('.fb-profile-stats > div')
    .first()
    .locator('dd');
...
test('ai saas: the overview and the error state both work', async ({ page }) => {
  ...
  await expect(
    ...
    /longer than this template accepts/i
  );
...
test('web game: sound is off by default, toggles, and persists', async ({ page }) => {
  await page.goto(
    '/showcase/playable-web-game/preview'
  );
...
test('showcase surfaces stay readable in every Xroga theme', async ({ page }) => {
  for (const theme of THEMES) {
    ...
    expect(
      contrast,
      `${theme} contrast could not be measured`
    ).not.toBeNull();

    expect(
      contrast!,
      `${theme} heading is not readable against the card surface`
