...
test('real Supabase login persists, Operations works, cross-tenant access is denied, and logout clears the session', async ({ page }) => {
  ...
  let billingCheckout: 'not_requested' | 'verified_whop_configuration' = 'not_requested';
  if (launchBillingApiUrl) {
    ...
    expect(billingStatus).toMatchObject({
      plan: 'free',
      publicPlanName: 'Free',
      isPaid: false,
      billingProvider: null,
    });
    const checkoutResponse = await fetch(`${launchBillingApiUrl}/api/billing/checkout`, {
      method: 'POST',
    ...
    expect(checkout.checkoutConfigurationId).toEqual(expect.any(String));
    const checkoutUrl = new URL(checkout.purchaseUrl ?? '');
    expect(checkoutUrl.protocol).toBe('https:');
    expect(checkoutUrl.hostname === 'whop.com' || checkoutUrl.hostname.endsWith('.whop.com')).toBe(true);
    billingCheckout = 'verified_whop_configuration';
  }
