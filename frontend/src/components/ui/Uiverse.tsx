...
export function LogoutButton({ onClick }: { onClick: () => void }) {
  return (
    <button type="button" onClick={onClick} className="xv-logout-btn" aria-label="Logout">
      {/* The arrow winds back before it leaves, which is what makes it read as going
          somewhere rather than as a static arrow that happens to have moved. The solid
          Font Awesome mark it replaces was the last filled glyph in this menu, drawn at
          a different weight from everything around it. */}
      <div className="xv-sign">
        <AnimatedIcon icon={LogoutIcon} size={17} intro={false} />
      </div>
