...
const SHINE_LABEL = `Xroga AI ${XROGA_MODEL_NAME} ${XROGA_MODEL_VERSION}`;

/** Uiverse andrew-manzyk — morphing wait mark */
const PATH_MORPH =
...
interface MorphWaitLoaderProps {
  className?: string;
  showLabel?: boolean;
  size?: 'sm' | 'md';
}
...
/**
 * Compact morphing wait icon (Uiverse andrew-manzyk) + shine label (neerajbaniwal).
 * Terminal AI processing — black hole visual lives on the homepage section only.
 */
export function MorphWaitLoader({
  className,
  showLabel = true,
  size = 'sm',
}: MorphWaitLoaderProps) {
  return (
    <div
      className={cn(
        'xv-morph-wait',
        size === 'sm' ? 'xv-morph-wait--sm' : 'xv-morph-wait--md',
        className
      )}
      role="status"
      aria-label={SHINE_LABEL}
    >
      <div className="xv-morph-loader" aria-hidden>
      ...
      {showLabel ? <span className="xv-morph-shine">{SHINE_LABEL}</span> : null}
    </div>
  );
}
