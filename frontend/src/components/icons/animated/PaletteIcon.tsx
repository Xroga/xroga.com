...
const DOTS_GROUP_VARIANTS: Variants = {
  normal: {},
  animate: {
    transition: { delayChildren: DRAW_DURATION, staggerChildren: DOT_STAGGER },
  },
};

const DOT_VARIANTS: Variants = {
  normal: { scale: 1, transition: { duration: 0.2 } },
  // Two keyframes only: motion rejects 3+ keyframes on a spring. Lower damping (10)
  // = weaker restoring force = a bigger, slower overshoot before it settles, versus
  // damping 14 which snapped back to 1 too quickly.
  animate: {
    scale: [0, 1],
    ...
    return (
        ...
        >
          ...
          >
            ...
            />
            <m.g animate={controls} initial="normal" variants={DOTS_GROUP_VARIANTS}>
              {DOTS.map((dot) => (
                <m.circle
                  cx={dot.cx}
                  cy={dot.cy}
