...
import { artifactPresentation, type UniversalOutput } from '@/lib/universalOutput';
  ...
  return (
    <section className="space-y-3 py-2 text-sm text-[var(--foreground)]" aria-label="Xroga output">
      ...
      {output.artifacts.map((artifact) => {
        ...
        const source = artifact.uri || (artifact.inline && `data:${artifact.mediaType};base64,${artifact.inline}`);
        return (
          <article key={artifact.id} className="rounded-xl border border-[var(--border)] p-3">
            ...
            {artifact.uri && presentation === 'download' ? <a href={artifact.uri} className="mt-2 inline-block underline" download>Download artifact</a> : null}
            ...
            {!source ? <p className="mt-2 text-xs text-[var(--muted)]">Artifact is recorded but has no inline preview.</p> : null}
          </article>
        );
      })}
