...
import { getSelectedRepoContext } from './repoContext';
...
function githubTodoLabel(hasSelectedRepo: boolean): string {
  return hasSelectedRepo ? 'Using your selected GitHub repo' : 'Prepare sandbox (GitHub optional)';
}

/** Context-aware todos — must match what the user asked for, not a generic SaaS checklist. */
export function buildTodosForPrompt(userPrompt: string, opts?: { hasSelectedRepo?: boolean }): BuildTodoDef[] {
  ...
  const gh = githubTodoLabel(hasSelectedRepo);
  ...
  if (update && !hackathon) {
    return [
      { id: 'github', label: hasSelectedRepo ? 'Load files from selected repo' : 'Load project files' },
  ...
  if (chatbot) {
    return [
      ...
      { id: 'code-gen', label: 'Build HelpBee chat UI + send handlers' },
  ...
  if (landing || blog) {
    ...
    return [
      ...
      { id: 'plan', label: `Plan ${name} pages & night/day or theme toggles` },
...
export function seedBuildTodos(userPrompt = ''): SwarmTodoItem[] {
  ...
  // Never leave users stuck on "Connect GitHub" when a repo is already selected
