...
function resolveApiUrl(): string {
  ...
  if (configured) {
    try {
      ...
      console.error(
        '[config] Ignoring invalid public API origin',
      );
    } catch {
      console.error(
        '[config] Ignoring malformed public API origin',
      );
    }
  }
...
export interface StreamSwarmOptions {
  ...
  /**
   * Reuse a session token already fetched —
   * skips a second Supabase round-trip.
   */
  ...
  clientMeta?: {
    ...
    /**
     * Explicit same-run resume.
     *
     * When true, runId must be the durable interrupted run ID rather
     * than a newly generated UUID.
     */
    resumeRun?: boolean;
    ...
    semanticGoalContract?:
      SemanticRequestPlan['goalContract'];

    /**
     * Visibility for a repository this build creates.
     *
     * Omitting this means private.
     */
    githubVisibility?:
      | 'private'
      | 'public';
...
async function waitForPersistedSwarmRun(
  ...
  options.onProgress?.({
    ...
    message:
      'Build continues safely in the background. Reconnecting to the persisted run...',
  });

  /*
   * This timeout controls how long this particular browser reconnect
   * request waits.
   *
   * It does NOT stop Agent V2 or cancel the durable backend build.
   */
  const deadline =
    Date.now() +
    30 * 60_000;
  ...
  while (
    ...
    try {
      response =
        await fetch(
          `${API_URL}/api/swarm/runs/${runId}?afterSequence=${Math.max(
            0,
            lastSequence,
          )}`,
    ...
    if (
      ...
      if (
        ...
        throw new ApiError(
          'Your session expired while reconnecting.',
          401,
        );
      }
  ...
  throw new ApiError(
    'The build is still running, but reconnect timed out. You can safely return later.',
...
export async function streamSwarmExecute(
  ...
  /*
   * Fresh requests get a fresh run ID.
   *
   * Explicit interrupted-build recovery supplies the previous durable
   * run ID so backend checkpoint recovery can find the SAME Agent V2 run.
   */
  const clientRunId =
    options.runId ??
    (
  ...
  if (
    ...
    if (
      ...
      throw new Error(
        data.error ??
        'Authentication failed — sign out and sign in again to refresh your session.',
      );
    }
    ...
    if (
      ...
      throw new ApiError(
        data.error ??
        'Out of tokens — upgrade your plan to continue.',
  ...
  if (
    ...
    throw new Error(
      'No response body from swarm stream',
    );
  }
  ...
  /*
   * This detects a dead HTTP/SSE connection.
   *
   * It does NOT stop the persisted backend build.
   * When the stream stalls, Xroga switches to durable run polling.
   */
  ...
  try {
    while (
      ...
      try {
        ...
        throw new Error(
          'The build service is not responding. Please try again.',
        );
      }
...
let authCacheListenerStarted =
  false;

function updateAccessTokenCache(
  session:
    Session |
    null,
): void {
...
/**
 * Read any chat attachment as a data URL.
 */
  ...
  terminalSessions: {
    list:
      (
        ...
        return apiFetch<
          CloudTerminalSessionPage
        >(
          qs
            ? `/api/terminal-sessions?${qs}`
    ...
    get:
      (
        ...
        apiFetch<{
          ...
          `/api/terminal-sessions/${encodeURIComponent(
            id,
          )}`,
        ),
    ...
    revoke:
      (
        ...
        apiFetch<{
          ...
          `/api/message-shares/${encodeURIComponent(
            token,
          )}`,
  ...
  github: {
    oauthUrl:
      () => {
        ...
        return apiFetch<{
          ...
          `/api/github/oauth?redirect_uri=${encodeURIComponent(
            redirectUri,
          )}`,
        );
      },
    ...
    repoState:
      (
        ...
        >(
          `/api/github/repo-state?repoName=${encodeURIComponent(
            repoName,
    ...
    getBuildFiles:
      (
        ...
        apiFetch<{
          ...
          `/api/github/build-files?repoName=${encodeURIComponent(
            repoName,
          )}`,
        ),
    ...
    analyzeRepo:
      (
        ...
        apiFetch<{
          ...
          `/api/github/analyze?repoName=${encodeURIComponent(
            repoName,
          )}${
            branch
              ? `&branch=${encodeURIComponent(
    ...
    oauthUrl:
      () => {
        ...
        return apiFetch<{
          ...
          `/api/vercel/oauth?redirect_uri=${encodeURIComponent(
            redirectUri,
          )}`,
        );
      },
    ...
    listDomains:
      (
        ...
        apiFetch<{
          ...
          `/api/vercel/domains?project=${encodeURIComponent(
            project,
          )}`,
        ),
    ...
    oauthUrl:
      () => {
        ...
        return apiFetch<{
          ...
          `/api/supabase/oauth?redirect_uri=${encodeURIComponent(
            redirectUri,
          )}`,
        );
      },
  ...
  integrations: {
    ...
    deleteProviderKey:
      (
        ...
        apiFetch(
          `/api/integrations/provider-keys/${encodeURIComponent(
            provider,
          )}`,
    ...
    syncVercelEnv:
      (
        ...
        apiFetch<{
          ...
          '/api/integrations/sync-vercel-env',
    ...
    completeCwsOAuth:
      (
        ...
        apiFetch<{
          ...
          '/api/publish/cws-oauth/callback',
    ...
    syncPlayCredentials:
      () =>
        apiFetch<{
          ...
          '/api/publish/sync-play-credentials',
    ...
    syncAppleCredentials:
      (
        ...
        apiFetch<{
          ...
          '/api/publish/sync-apple-credentials',
    ...
    getRun:
      (
        ...
        apiFetch<
          SwarmRunSummary
        >(
          `/api/swarm/runs/${runId}?afterSequence=${Math.max(
            0,
            Math.floor(
              afterSequence,
            ),
    ...
    saveConversation:
      (
        ...
        apiFetch<{
          ...
          `/api/swarm/runs/${runId}/conversation`,
    ...
    entitlement:
      () =>
        apiFetch<{
          ...
          promotionActivationDeadline:
            string;

          requiresCard:
            boolean;

          autoChargesAtPromotionEnd:
            boolean;
        }>(
    ...
    economics:
      () =>
        apiFetch<{
          ...
          freeUserMonthlyTokens:
            number;

          freeUserWorstCaseApiUsd:
            number;
  ...
  tokenDistribution: {
    preview:
      () =>
        ...
        >(
          '/api/token-distribution/preview',
        ),
    ...
    confirm:
      (
        ...
        apiFetch<{
          ...
          '/api/token-distribution/confirm',
