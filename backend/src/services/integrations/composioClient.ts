...
export interface XrogaConnectToolDetails
  extends XrogaConnectTool {
  name?: string;
...
export async function listConnectedComposioToolkits(
  ...
  const response =
    await composioRequest<ComposioSessionToolkitResponse>(
      `/tool_router/session/${encodeURIComponent(
        sessionId,
...
export async function getComposioToolDetails(
  ...
  const details =
    await composioRequest<ComposioToolDetailsResponse>(
  ...
  if (!toolkit) {
    throw new ComposioClientError(
      'Composio did not return tool metadata.',
      {
        status: 502,
        code:
          'COMPOSIO_TOOL_METADATA_MISSING',
      },
    );
  }
  ...
  return {
    ...
    scopes:
      Array.isArray(
        ...
        : [],
    scopeRequirements:
      details.scope_requirements,
    tags,
    noAuth:
      details.no_auth ===
      true,
    deprecated:
      details.is_deprecated ===
      true,
    version:
      details.version,
    risk,
...
export async function executeComposioReadTool(
  ...
  return executeDiscoveredComposioTool(
    userId,
    {
      ...input,
      toolSlug,
  ...
  if (
    ...
    throw new ComposioClientError(
      'Xroga Connect action authorization is required.',
      {
        status: 409,
        code:
          'COMPOSIO_ACTION_CONFIRMATION_REQUIRED',
      },
    );
  }
