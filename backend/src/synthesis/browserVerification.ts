...
const IGNORABLE_CONSOLE = [
  ...
  /favicon\.ico/i,
  /react-devtools/i,
  // Chromium reports every failed subresource as a console error whose text does *not* name
  // the resource — "Failed to load resource: the server responded with a status of 404".
  // That is a strictly less-informed duplicate of something `networkFailures` already has,
  // with the URL, and judges properly there (a missing favicon is ignored, a failed script is
  // not). Judging the shadow copy as well would fail every page with a missing favicon while
  // the better-informed check correctly passed it.
  //
  // Found by the real-browser integration test; no fake would have produced this message.
