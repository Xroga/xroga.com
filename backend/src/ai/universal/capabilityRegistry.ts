...
export class CapabilityRegistry {
  ...
  resolve(requested: readonly string[], authorities: ReadonlySet<string>): {
    ...
    for (const id of [...new Set(requested)]) {
      ...
      if (!descriptor) { rejected.push({ id, reason: 'not_registered' }); continue; }
      if (descriptor.health() === 'unavailable') { rejected.push({ id, reason: 'unavailable' }); continue; }
      const missing = descriptor.requiredAuthorities.find((authority) => !authorities.has(authority));
      if (missing) { rejected.push({ id, reason: `missing_authority:${missing}` }); continue; }
