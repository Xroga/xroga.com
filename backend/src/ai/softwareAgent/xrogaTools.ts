...
export function createXrogaSoftwareTools(
  ...
  const writeFile = createTool({
    ...
    execute: async (rawInput) => {
      ...
      /*
       * We do not yet know whether this is logically a create or
       * modification.
       *
       * First enforce the common path allowlist.
       */
      const modifyDecision = evaluateWritePolicy(
      ...
      if (!modifyDecision.allowed) {
        const createDecision = evaluateWritePolicy(
          contract.writePolicy,
          'create',
          input.path,
        );
  ...
  const runChecks = createTool({
    ...
    execute: async () => {
      ...
      try {
        const checks = await host.runChecks(
          contract,
        );
  ...
  const startPreview = createTool({
    ...
    execute: async () => {
      ...
      try {
        const preview =
          await host.startPreview(contract);
  ...
  const createReviewBranch = createTool({
    ...
    execute: async () => {
      ...
      try {
        const result =
          await host.createReviewBranch(contract);
  ...
  const completeTask = createTool({
    ...
    execute: async (rawInput) => {
       ...
       * IMPORTANT:
       *
       * Cline completion tools end the agent run once executed.
       * Therefore we must NEVER expose complete_task to the
       * production Agent until the completion lifecycle is
       * wired to support rejection/continuation.
       *
       * For this migration slice this tool remains useful for
       * isolated tests only.
       */
      if (!result.complete) {
        return {
  ...
  return {
    ...
    /*
     * Deliberately separate until we wire safe completion
     * continuation in the actual Agent executor.
     */
