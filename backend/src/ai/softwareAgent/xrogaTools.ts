...
function upsertChangedFile(
  ...
  const existing = evidence.changedFiles.find(
    (file) => file.path === next.path,
  );

  if (!existing) {
    evidence.changedFiles.push(next);
    return;
  }

  Object.assign(existing, {
    ...next,
    created: existing.created === true || next.created === true,
  });
}
...
export function createXrogaSoftwareTools(
  ...
  const listFiles = createTool({
    ...
    execute: async () => {
      try {
        const files = await host.listFiles(contract);

        await emit({
          type: 'project.inspect.completed',
          status: 'success',
          title: 'Inspected project',
          summary: `${files.length} file${files.length === 1 ? '' : 's'} available`,
        });
        ...
        return {
          ...
          message: 'Xroga could not list the project files.',
        };
      }
    },
  });
  ...
  const readFiles = createTool({
    ...
    execute: async (rawInput) => {
      const toolInput = rawInput as {
        paths: string[];
      };

      try {
        const files = await host.readFiles(
          contract,
          toolInput.paths,
        );
        ...
        return {
          ...
          message: 'Xroga could not read one or more requested project files.',
        };
      }
    },
  });

  const searchProject = createTool({
    name: 'project_search',
    description:
    ...
    execute: async (rawInput) => {
      ...
      try {
        const results = await host.searchProject(
          contract,
          toolInput.query,
        );
  ...
  const writeFile = createTool({
    ...
    execute: async (rawInput) => {
      ...
      if (!decision.allowed) {
        return {
          ok: false,
          code: decision.code ?? 'WRITE_SCOPE_DENIED',
          message:
            decision.message ??
      ...
      try {
        ...
        const code =
          message.startsWith('CREATE_CONFLICT:')
            ? 'CREATE_CONFLICT'
            : message.startsWith('MODIFY_TARGET_MISSING:')
  ...
  const deleteFile = createTool({
    ...
    execute: async (rawInput) => {
      ...
      if (!decision.allowed) {
        return {
          ok: false,
          code: decision.code ?? 'DELETE_SCOPE_DENIED',
          message:
            decision.message ??
      ...
      try {
        ...
        return {
          ...
          message: 'Xroga could not safely delete this file.',
        };
      }
    },
  });
    ...
    execute: async (rawInput) => {
      ...
      try {
        ...
        return {
          ...
          message: 'The command could not be executed in the Xroga sandbox.',
        };
      }
    },
  });

  const runChecks = createTool({
    name: 'run_checks',
    description:
    ...
    execute: async () => {
      await emit({
        type: 'check.started',
        status: 'running',
        title: 'Running project checks',
      });

      try {
        const checks = await host.runChecks(contract);
        ...
        await emit({
          ...
          title: failed.length === 0
            ...
            : `${failed.length} project check${failed.length === 1 ? '' : 's'} failed`,
        });
        ...
        return {
          ...
          message: 'Xroga could not complete the project checks.',
        };
      }
    },
  });
  ...
  const gitDiff = createTool({
    ...
    execute: async () => {
      try {
        const diff = await host.gitDiff(contract);
        ...
        return {
          ok: false,
          code: 'DIFF_FAILED',
          message: 'Xroga could not calculate the project diff.',
        };
      }
    },
  });

  const startPreview = createTool({
    name: 'start_preview',
    description:
    ...
    execute: async () => {
      if (contract.preview === 'not_applicable') {
        return {
          ok: false,
          code: 'PREVIEW_NOT_APPLICABLE',
          message: 'This software task does not require a web Preview.',
        };
      }
      ...
      try {
        const preview = await host.startPreview(contract);
        ...
        return {
          ...
          message: 'The application Preview could not start.',
        };
      }
    },
  });

  const verifyPreview = createTool({
    name: 'verify_preview',
    description:
    ...
    execute: async (rawInput) => {
      ...
      await emit({
        type: 'browser.verification.started',
        status: 'running',
        title: 'Verifying Preview',
        evidence: {
          previewId: toolInput.preview_id,
        },
      });

      try {
        const preview = await host.verifyPreview(
          contract,
        ...
        const valid =
          typeof preview.httpStatus === 'number' &&
          preview.httpStatus >= 200 &&
          preview.httpStatus < 400 &&
          preview.consoleErrors.length === 0 &&
          preview.runtimeErrors.length === 0 &&
          (
        ...
        return {
          ...
          message: 'Xroga could not complete Preview verification.',
        };
      }
    },
  });
    ...
    execute: async () => {
      if (!contract.repository) {
        return {
          ok: false,
          code: 'NO_REPOSITORY',
          message: 'There is no authorized repository for this task.',
        };
      }

      if (contract.persistence !== 'review_branch') {
        return {
          ok: false,
          code: 'REPOSITORY_PERSISTENCE_NOT_AUTHORIZED',
          message: 'This task is not authorized to persist repository changes.',
        };
      }

      const verification = evaluateSoftwareCompletion({
        previewRequirement: contract.preview,
        evidence,
      ...
      if (!verification.complete) {
        return {
          ...
          message: 'Xroga will not persist unverified software changes.',
          blockers: verification.blockers,
        };
      }

      try {
        const result = await host.createReviewBranch(contract);

        evidence.branch = result.branch;
        evidence.commitSha = result.commitSha;

        await emit({
          type: 'git.branch.created',
          status: 'success',
          title: `Prepared ${result.branch}`,
        });

        await emit({
          type: 'git.commit.created',
          status: 'success',
          title: 'Saved verified changes',
          evidence: {
        ...
        return {
          ok: false,
          code: 'REPOSITORY_PERSIST_FAILED',
          message: 'Xroga could not persist the verified changes.',
        };
      }
    },
  });
  ...
  const completeTask = createTool({
    ...
    execute: async (rawInput) => {
      ...
      const result = evaluateSoftwareCompletion({
        previewRequirement: contract.preview,
        evidence,
        requireSuccessfulChecks: true,
        requireRepositoryPersistence:
          contract.persistence === 'review_branch',
      });
      ...
      if (!result.complete) {
        return {
          ...
          message: 'The task is not verified yet.',
        };
      }
