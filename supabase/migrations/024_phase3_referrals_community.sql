-- Phase 3: Referrals, Community Pool, Token Distribution
...
CREATE INDEX IF NOT EXISTS idx_referral_profiles_code ON public.user_referral_profiles (code);
...
CREATE TABLE IF NOT EXISTS public.referral_rewards (
  ...
  status TEXT NOT NULL DEFAULT 'rewarded' CHECK (status IN ('rewarded', 'pending')),
...
CREATE INDEX IF NOT EXISTS idx_community_pool_requests_user ON public.community_pool_requests (user_id, created_at DESC);
...
ALTER TABLE public.community_pool_requests ENABLE ROW LEVEL SECURITY;
